<?php
    ...
    if(mysqli_connect_error()){
        ...
        // Extrai o valor do usuário da matriz de dados decodificados.
        $user = $dData['user'];

        // Inicializa uma variável para armazenar o resultado da operação.
        $result = "";

        // Verifica se o valor do usuário não está vazio.
        if($user != ""){
            // Monta a consulta SQL para selecionar todos os registros da tabela "users" onde o valor da coluna "user" é igual ao valor do usuário fornecido.
            $sql = "SELECT * FROM users WHERE user = '$user';";

            // Executa a consulta SQL usando a conexão com o banco de dados.
            $res = mysqli_query($conn, $sql);

            // Verifica se a consulta retornou algum resultado (ou seja, se o nome de usuário já está em uso).
            if(mysqli_num_rows($res) != 0){
                $result = "O nome de usuário já está em uso!";
            }
            else{
                $result = "";
            }
